...
DUBBO_PORT="20896"
APPFRAME_RAC_SERVER_NAME="BALANCE-SERVER"
PROCESS_NAME="DubboServiceStart"
PROCESS_PARM="resources.dubbo.protocol.port=$DUBBO_PORT"
CUR_USER=`whoami`
CENTER_NAME=balance-services
PRODUCT_CENTER_BASE_PATH=${HOME}/applications/balance-services
...
if [ "$1" = "start" ]; then
	...
	source "${HOME}/bin/balance-server/setEnv.sh"
	...
	java ${MEM_ARGS} -Dappframe.server.name=$APPFRAME_RAC_SERVER_NAME -Dresources.dubbo.protocol.port=$DUBBO_PORT  ${JAVA_OPTIONS} com.x.sdk.appserver.DubboServiceStart ${CENTER_NAME} >> $HOME/logs/balance-center-dubbo-$DUBBO_PORT.log& 2>&1&
        echo "ibss resources center dubbo server started!! process param is [$PROCESS_PARM].logs at $HOME/logs/balance-center-dubbo-$DUBBO_PORT.log"
else if [ "$1" = "stop" ]; then
        PROCESS_NUMBER=0
